...
const Header = (props) => {
  const [menuOpen , setMenuOpen] = useState(false);
    ...
    const changePage=(e, destination) => {
      e.preventDefault();
      gsap.to(".menu__item1",{ duration:.2, opacity:0, y:"-100%"})
      gsap.to(".menu__item2",{ duration:.2, opacity:0, x:"-170%"})
      gsap.to(".menu__item3",{ duration:.2, opacity:0, x:"-100%"})
      gsap.to(".menu__item4",{ duration:.2, opacity:0, x:"-100%"})
      gsap.to(".menu__item5",{ duration:.2, opacity:0, y:"-100%"})
      gsap.to(".menu-box", { duration: 1, css: { display: "none", zIndex:0 } })
